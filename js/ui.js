...
function renderPills() {
  ...
  selectedEffects.forEach((e) => {
    ...
    btn.textContent = '×';
    btn.onclick = () => {
      selectedEffects = selectedEffects.filter((x) => x !== e);
      populateAddEffect();
      renderPills();
      run();
    };
...
if (searchWorker) {
  ...
  searchWorker.onerror = () => {
    searchWorker = null;
    run();
  }; // worker failed to load — retry synchronously
}
...
function run() {
  ...
  if (searchWorker) {
    ...
    showSearching();
    setTimeout(() => {
      if (id === queryId) render(search(startState, targets), ctx);
    }, 50);
  }
}
...
function render(result, { baseName, def, startState }) {
  ...
  if (startState.length === 0) {
    html += `<span class="eff" style="opacity:.5">— no starting effect —</span>`;
  }
  ...
  steps.forEach((s, idx) => {
    ...
    if (removed.length)
      html += `<span class="stepdelta">(${removed.join(', ')} → ${added.join(', ') || '—'})</span>`;
